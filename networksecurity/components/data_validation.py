...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            trained_data=DataValidation.read_data(train_file_path)
            test_data=DataValidation.read_data(test_file_path)

            ## Validate number of columns
            status=self.validation_number_of_columns(dataframe=trained_data)
            if not status:
                error_message=f"Train dataframe does not contain all columns.\n"
            status=self.validation_number_of_columns(dataframe=test_data)
            if not status:
                error_message=f"Test dataframe does not contain all columns.\n"
            
            ## Let's check data drift
            status=self.detect_data_drift(base_df=trained_data,current_df=test_data)
